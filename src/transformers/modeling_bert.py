...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        #     nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(3, 3), stride=(3, 3), padding=0),
        #     nn.BatchNorm2d(32),
        #     nn.ReLU(),
        #     nn.MaxPool2d(2, 2),
        #     nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3, 1), stride=(3, 1), padding=0),
        #     nn.BatchNorm2d(64),
        #     nn.ReLU(),
        #     nn.MaxPool2d(2, 2),
        #     )
        ...
        # bert+all_layer+cnn
        self.cnn = nn.Sequential(
            nn.Conv2d(in_channels=13, out_channels=26, kernel_size=(3, 3), stride=(3, 3), padding=0),
            nn.BatchNorm2d(26),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(in_channels=26, out_channels=52, kernel_size=(3, 3), stride=(3, 3), padding=0),
            nn.BatchNorm2d(52),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
        )
    ...
    Examples::
        ...
        # pooled_output = self.classifier1(self.dropout(cls_output))
        # logits = self.classifier2(pooled_output)

        all_layer = torch.transpose(torch.cat(tuple([t.unsqueeze(0) for t in outputs[2]]), 0), 0, 1)
        pooled_output = self.cnn(all_layer)
        pooled_output = pooled_output.flatten(start_dim=1)
        concat = self.dropout(pooled_output)
